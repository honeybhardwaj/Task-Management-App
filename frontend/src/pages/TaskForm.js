...
const TaskForm = () => {
  ...
  const localDueDate = new Date(task.dueDate);
  localDueDate.setHours(23, 59, 59, 999); // end of day in local time
  ...
  id ? await API.put(`/tasks/${id}`, adjustedTask) : await API.post('/tasks', adjustedTask);
  navigate('/');
};
